...
const ListPage = () => {
  ...
  if (isLoading) return <div className="text-center py-20 text-gray-500">원고 목록을 불러오는 중...</div>;
  if (isError) return <div className="text-center py-20 text-red-500">목록을 가져오지 못했습니다.</div>;

  return (
    <section>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-extrabold text-gray-900">연재 목록</h1>
        <span className="text-sm text-gray-500">총 {episodes?.length || 0}화</span>
      </div>
            ...
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-gray-400 text-sm mt-2 font-light">
                  {new Date(ep.created_at).toLocaleDateString()} · 조회수 {ep.comments}
                </p>
              </div>
